...
namespace React
{
	public class JsonSerializer
	{
		...
		public static object Decode (byte[] json)
		{
            ...
            return Decode(encoding.GetString(json, 0, json.Length));
		}
		...
		{
            ASCIIEncoding encoding = new ASCIIEncoding();
            return Decode<T>(encoding.GetString(json, 0, json.Length));
		}
		...
		static object PopulateObject (Type T, object obj, object instance)
		{
			...
			if (T.IsAssignableFrom (obj.GetType ())) {
				...
				instance = Convert.ChangeType (obj, T);
			} else if (T.IsEnum) {
				instance = System.Enum.Parse (T, (string)obj);
		...
		protected static object ParseObject (char[] json, ref int index, ref bool success)
		{
			...
			var typestring = (string)table ["__type"];
			if (typestring == "Reactable")
			{
#if UNITY_EDITOR
				return (Reactable)AssetDatabase.LoadAssetAtPath((string)table ["path"], typeof(Reactable));
#else
                string path = (string)table ["path"];
			    path = path.Replace("Assets/Resources/","").Replace(".asset","");
                var reactable = (Reactable)Resources.Load(path, typeof(Reactable));
                if (reactable == null) Debug.LogError("[React JsonSerializer] Couldn't load Reactable at " + path);
                return reactable;
#endif
			}  
			var type = Type.GetType (typestring);
			var instance = Activator.CreateInstance (type);
			PopulateObject (type, table, instance);
		...
		protected static object ParseNumber (char[] json, ref int index, ref bool success)
		{
			...
			success = double.TryParse (token, NumberStyles.Any, CultureInfo.InvariantCulture, out number);
			index = lastIndex + 1;
			return number;
		}
		...
		protected static bool SerializeValue (object value, StringBuilder builder)
		{
			...
			if (value is string) {
				...
				success = SerializeString (System.Enum.GetName (value.GetType (), value), builder);
			...
			} else if (value is Reactable) {
#if UNITY_EDITOR				
				var r = value as Reactable;
				var h = new Hashtable ();
				h ["__type"] = value.GetType ().ToString ();
				h ["name"] = r.name;
				h ["path"] = AssetDatabase.GetAssetPath (r);
				h ["GUID"] = AssetDatabase.AssetPathToGUID (AssetDatabase.GetAssetPath (r));
				...
				foreach (var i in value.GetType ().GetFields ()) {
					if (i.FieldType == value.GetType ()) {
						Debug.LogError ("Cannot Serialize " + i.FieldType.ToString ());
						break;
					}
